...
import { Alert, Container, Fab, FormControlLabel, FormGroup, Grid, Paper, Snackbar, Switch } from '@mui/material';
...
const { singIn, clientesEmpresa } = rutasBackEnd
...
export function CustomerCompanyById() {
  ...
  useEffect(() => {
    ...
    console.log(`http://localhost:8080/${clientesEmpresa}/${id}`)

    axios.get<Customer>(`http://localhost:8080/${clientesEmpresa}/${id}`,
      { headers: { Authorization: `Bearer ${token}` } })
      .then(resp => {
        const { persona, empresa, estado } = resp.data
  ...
  const handleConfirmation = () => {
    ...
    axios.put(`http://localhost:8080/cliente/empresa/${id}`,
      body,
        ...
        <Grid item sm={12} md={12} lg={12}>
          ...
          >
            ...
            <Box sx={{ marginInline: 2 }}>
              <DataGridCustomer rows={rowsPerson} setRows={setRowsPerson} dniEditable={true} />
            </Box>
          </Paper>
        </Grid>
        <Grid item sm={12} md={12} lg={12}>
          ...
          >
            ...
            <Box sx={{ marginInline: 2 }}>
              <DataGridCompany rows={rowsCompany} setRows={setRowsCompany} cuiEditable={false} />
            </Box>
          </Paper>
        </Grid>
      </Grid>
